	...
	<div id = "center">
		...
		<div class="inputgo">
			<form action="./join" method='POST'>
				<div>
					<div class="grid">
						<span class="gridMid"> 성<input type="text" name = 'sName1'>	</span>
						<span class="gridMid2"> 이름 <input type="text" name = 'sName2'> </span>
						 
					
						<div class="warning">
							<% if(request.getAttribute("nameError1") != null){ %>
								<%=(String)request.getAttribute("nameError1") %>
							<%}else if(request.getAttribute("nameError3") != null){ %>
								<%=(String)request.getAttribute("nameError3") %>
							<%}else if(request.getAttribute("nameError2") != null){ %>
								<%=(String)request.getAttribute("nameError2") %>
							<%} %>
						</div>
					</div>
		...
		<%-- 					<%=(String)request.getAttribute("nameError1") %> --%>
				<div>
					사용자이름<input type="text" name = 'sEmail'>
					<div class="warning">
						<% if(request.getAttribute("emailError") != null) { %>
							<%=(String)request.getAttribute("emailError")%>
						<%}else if(request.getAttribute("emailOver") != null){%>
						<%=(String)request.getAttribute("emailOver") %>
						<%} %>
					</div>
					문자,숫자,마침표를 사용할 수 있습니다.
				</div>		
				<div class="grid">
					<span class="gridMid"> 비밀번호<input type="password" name = 'sPwd'> </span>
					<span class="gridMid2"> 확인<input type="password" name = 'sPwd2'> </span>
